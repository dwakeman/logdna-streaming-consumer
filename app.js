 ...
 *   LOG_LEVEL:   Establishes the log4js log level
...
var opts = {};
//var topicName = 'activity-tracker-dallas-dw-streaming';
...
if (process.env.VCAP_SERVICES) {
    logger.debug("Using VCAP_SERVICES to find credentials.");
    ...
    logger.debug("Using command line arguments to find credentials.");

    if (process.argv.length < 5) {
        logger.debug('ERROR: It appears the application is running is running without VCAP_SERVICES but the arguments are incorrect for local mode.');
        logger.debug('\nUsage:\n' +
    ...
    if (! fs.existsSync(opts.calocation)) {
        logger.error('Error - Failed to access <cert_location> : ' + opts.calocation);
        process.exit(-1);
    }
...
logger.debug("Kafka Endpoints: " + opts.brokers);
logger.debug("Kafka Options: " + JSON.stringify(opts));

if (!opts.hasOwnProperty('brokers') || !opts.hasOwnProperty('api_key') || !opts.hasOwnProperty('calocation')) {
    logger.error('Error - Failed to retrieve options. Check that app is bound to an Event Streams service or that command line options are correct.');
    process.exit(-1);
}
...
logger.debug("Driver options: " + JSON.stringify(driver_options));
var admin_opts = {
...
runLoops();
logger.info("This sample app will run until interrupted.");

// Build and start the producer/consumer
function runLoops() {
    logger.debug("entering function runLoops() in app.js....");
    var consumer_opts = {
        'client.id': 'activity-tracker-dallas-dw-streaming-consumer',
        'group.id': 'activity-tracker-dallas-dw-streaming-group'
    };
/*
    var producer_opts = {
...
//        producer_opts[key] = driver_options[key];
    ...
    logger.debug("in runLoops...  Starting the clients....")
    // Start the clients
    if (runConsumer) {
